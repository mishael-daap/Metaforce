...
router.get('/', async (req, res) => {
  try {
    ...
    res.json({ success: true, error: null, components: objects });
  } catch (err) {
...
router.put('/:apiName', async (req, res) => {
  try {
    ...
    if (!objectSpec || !objectSpec.fullName) {
      res.status(400).json({ success: false, error: 'object spec with fullName is required in request body', components: [] });
      return;
    }
    ...
    const createResult = await createCustomObject(projectId, objectSpec);
    if (!createResult.success) {
      res.status(500).json({ success: false, error: `Object update failed: ${createResult.error}`, components: [] });
      return;
    }
...
router.delete('/:apiName', async (req, res) => {
  try {
    ...
    if (!deleteResult.success) {
      res.status(500).json({ success: false, error: `Delete from org failed: ${deleteResult.error}`, components: [] });
      return;
    }
