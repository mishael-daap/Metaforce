...
export async function createCustomObject(
  ...
  try {
    ...
    // Create object directory if it doesn't exist (preserve fields)
...
export async function deleteCustomObject(
  ...
  try {
    ...
    if (!fs.existsSync(objectDir)) {
      return {
        success: false,
        error: `Custom object '${objectName}' not found in project '${projectId}'.`,
      };
    }
...
function validateInputs(
  ...
  if (!validDeploymentStatuses.includes(spec.deploymentStatus)) {
    return {
      valid: false,
      error: `deploymentStatus must be one of: ${validDeploymentStatuses.join(", ")}`,
    };
  }
  ...
  if (!validSharingModels.includes(spec.sharingModel)) {
    return {
      valid: false,
      error: `sharingModel must be one of: ${validSharingModels.join(", ")}`,
    };
  }
  ...
  if (!validNameFieldTypes.includes(spec.nameField.type)) {
    return {
      valid: false,
      error: `nameField.type must be one of: ${validNameFieldTypes.join(", ")}`,
    };
  }
