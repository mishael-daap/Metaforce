...
export function buildCustomFieldXml(spec) {
    const lines = [
...
function buildTextFieldXml(spec) {
    const lines = [];
...
function buildTextAreaFieldXml(spec) {
    const lines = [];
    if (spec.required !== undefined) {
        lines.push(`    <required>${spec.required ? 'true' : 'false'}</required>`);
    }
...
function buildLongTextAreaFieldXml(spec) {
    const lines = [];
    if (spec.length !== undefined) {
        lines.push(`    <length>${spec.length}</length>`);
    }
...
function buildHtmlFieldXml(spec) {
    const lines = [];
    if (spec.length !== undefined) {
        lines.push(`    <length>${spec.length}</length>`);
    }
...
function buildEncryptedTextFieldXml(spec) {
    const lines = [];
    if (spec.externalId !== undefined) {
        lines.push(`    <externalId>${spec.externalId ? 'true' : 'false'}</externalId>`);
    }
...
function buildNumberFieldXml(spec) {
    const lines = [];
    if (spec.externalId !== undefined) {
        lines.push(`    <externalId>${spec.externalId ? 'true' : 'false'}</externalId>`);
    }
...
function buildCurrencyFieldXml(spec) {
    const lines = [];
    lines.push(`    <precision>${spec.precision}</precision>`);
    if (spec.required !== undefined) {
        lines.push(`    <required>${spec.required ? 'true' : 'false'}</required>`);
...
function buildPercentFieldXml(spec) {
    const lines = [];
    lines.push(`    <precision>${spec.precision}</precision>`);
    if (spec.required !== undefined) {
        lines.push(`    <required>${spec.required ? 'true' : 'false'}</required>`);
...
function buildLocationFieldXml(spec) {
    const lines = [];
...
function buildCheckboxFieldXml(spec) {
    const lines = [];
...
function buildEmailFieldXml(spec) {
    const lines = [];
    if (spec.externalId !== undefined) {
        lines.push(`    <externalId>${spec.externalId ? 'true' : 'false'}</externalId>`);
    }
...
function buildPhoneFieldXml(spec) {
    const lines = [];
    if (spec.required !== undefined) {
        lines.push(`    <required>${spec.required ? 'true' : 'false'}</required>`);
    }
...
function buildUrlFieldXml(spec) {
    const lines = [];
    if (spec.required !== undefined) {
        lines.push(`    <required>${spec.required ? 'true' : 'false'}</required>`);
    }
...
function buildAutoNumberFieldXml(spec) {
    const lines = [];
...
function buildDateFieldXml(spec) {
    const lines = [];
    if (spec.required !== undefined) {
        lines.push(`    <required>${spec.required ? 'true' : 'false'}</required>`);
    }
...
function buildDateTimeFieldXml(spec) {
    const lines = [];
    if (spec.required !== undefined) {
        lines.push(`    <required>${spec.required ? 'true' : 'false'}</required>`);
    }
...
function buildTimeFieldXml(spec) {
    const lines = [];
    if (spec.required !== undefined) {
        lines.push(`    <required>${spec.required ? 'true' : 'false'}</required>`);
    }
...
function buildLookupFieldXml(spec) {
    const lines = [];
...
function buildMasterDetailFieldXml(spec) {
    const lines = [];
    lines.push(`    <referenceTo>${spec.referenceTo}</referenceTo>`);
    lines.push(`    <relationshipLabel>${escapeXml(spec.relationshipLabel)}</relationshipLabel>`);
    lines.push(`    <relationshipName>${spec.relationshipName}</relationshipName>`);
...
function buildPicklistFieldXml(spec) {
    const lines = [];
    if (spec.required !== undefined) {
        lines.push(`    <required>${spec.required ? 'true' : 'false'}</required>`);
    }
...
function buildMultiselectPicklistFieldXml(spec) {
    const lines = [];
    if (spec.required !== undefined) {
        lines.push(`    <required>${spec.required ? 'true' : 'false'}</required>`);
    }
...
function buildFormulaFieldXml(spec) {
    const lines = [];
...
function buildRollupSummaryFieldXml(spec) {
    const lines = [];
...
function buildValueSetXml(valueSet) {
    const lines = [];
    lines.push(`    <valueSet>`);
...
//# sourceMappingURL=customFieldBuilder.js.map