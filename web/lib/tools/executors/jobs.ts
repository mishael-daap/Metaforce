...
export async function getJobsByRequirementId(
  projectId: string,
  ...
  if (error) {
    throw new Error(
      `Failed to get jobs by requirement id: ${error.message}`
    );
  }
  ...
  if (!data || data.length === 0) {
    return {
      success: true,
      message: "No jobs found for this requirement.",
      jobs: [],
    };
  }
  ...
  return {
    ...
    message:
      pending > 0 || inProgress > 0
        ? `There are ${pending} pending and ${inProgress} in-progress jobs for this requirement.`
        : `All jobs for this requirement are settled. ${completed} completed, ${failed} failed.`,
  };
}
